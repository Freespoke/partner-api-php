...
class FreespokeTest extends TestCase
{
    ...
    public function testIndexSendsExpectedPayloadAndAuthHeaders(): void
    {
        ...
        $this->assertArrayHasKey('test_mode', $payload);
        $this->assertFalse($payload['test_mode']);
    }

    public function testIndexSendsTestModeTrueWhenSet(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, ['Content-Type' => 'application/json'], json_encode([
    ...
    public function testGetEpochReturnsZeroWhenMissing(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, ['Content-Type' => 'application/json'], json_encode([], JSON_UNESCAPED_SLASHES)),
        ], $history);
    ...
    public function testGetEpochRethrowsRequestException(): void
    {
        $history = [];
        $client = $this->buildClient([
            new RequestException('boom', new Request('GET', 'https://api.example.test/v1/content/epoch')),
        ], $history);
    ...
    public function testGetIndexStatusParsesJob(): void
    {
        ...
        $this->assertSame(['@type' => 'type.googleapis.com/google.protobuf.StringValue', 'value' => 'xyz'], $result->result);
        ...
        $this->assertSame('xyz', $result->doc_id);
    ...
    public function testGetIndexStatusRethrowsRequestException(): void
    {
        $history = [];
        $client = $this->buildClient([
            new RequestException('boom', new Request('GET', 'https://api.example.test/v1/job/job-abc')),
        ], $history);
    ...
    public function testDeleteSendsURLAndID(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, ['Content-Type' => 'application/json'], json_encode([
                'id' => 'a' . str_repeat('0', 39),
            ])),
        ], $history);

        $result = $client->delete('https://example.com/article', 'a' . str_repeat('0', 39));

        $this->assertNotNull($result);
        $this->assertSame('a' . str_repeat('0', 39), $result->id);
        $this->assertCount(1, $history);
        $req = $history[0]['request'];
        $this->assertSame('DELETE', $req->getMethod());
        $this->assertSame('/v1/content', $req->getUri()->getPath());
        parse_str($req->getUri()->getQuery(), $query);
        $this->assertSame('https://example.com/article', $query['url']);
        $this->assertSame('a' . str_repeat('0', 39), $query['id']);
    }

    public function testDeleteOmitsIDWhenNotProvided(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, [], json_encode(['id' => 'b' . str_repeat('0', 39)])),
        ], $history);

        $client->delete('https://example.com/article');
    ...
    public function testDeleteReturnsNullOnErrorMessage(): void
    {
        $history = [];
        $client = $this->buildClient([
        ...
        $this->assertNull($client->delete('https://example.com/article'));
    }

    public function testGetIndexStatusExtractsDocIDFromStringValueResult(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, [], json_encode([
        ...
        $status = $client->getIndexStatus('job-1');
        $this->assertNotNull($status);
        $this->assertSame('doc-abc', $status->doc_id);
    }

    public function testGetIndexStatusLeavesDocIDNullWhenAbsent(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, [], json_encode([
                'job' => ['jobId' => 'job-1', 'status' => 'JOB_STATUS_PENDING'],
            ])),
        ], $history);

        $status = $client->getIndexStatus('job-1');
        $this->assertNotNull($status);
        $this->assertNull($status->doc_id);
    }

    public function testGetIndexStatusLeavesDocIDNullWhenAnyTypeIsNotStringValue(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, [], json_encode([
    ...
    public function testDeleteReturnsNullOnEmptyId(): void
    {
        $history = [];
        $client = $this->buildClient([
            new Response(200, [], json_encode(['id' => ''])),
        ], $history);
